...
| SDK-007 | Editor | `PdfPage` rotation is read-only — no `set_rotation()`/`rotate()` despite "Document Editor API" entitlement | high | open | test_editor.py::test_page_rotation_is_settable |
...
| SDK-011 | Annotations | `PdfAnnotation.get_rect()` returns an opaque native handle int, not geometry | high | open | test_annotations.py::test_get_rect_returns_readable_geometry |
...
| SDK-017 | Forms | `collection[i]` / `find_by_full_name` always return base `PdfFormField`, never the typed subtype | high | open | test_forms.py::test_field_is_typed_subtype |
